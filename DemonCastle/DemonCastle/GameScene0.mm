...
+(CCScene *) scene
{
	...
	GameScene0 *layer = [GameScene0 node];
	[scene addChild: layer];
    
    GameHud *hud = [GameHud node];
    [scene addChild: hud];
    layer.hud = hud;
    hud.gameLayer0 = layer;
	return scene;
}
...
-(id) init
{
	if( (self=[super init])) {
        ...
        longmoving=NO;
        timerIsLock=NO;
        tileMap = [CCTMXTiledMap tiledMapWithTMXFile:@"gamescene0.tmx"];
...
{
    CCTMXLayer *_background1 = [tileMap layerNamed:@"Background1"];
    float minX=min(-30,max(-47,-(625-player.position.x)/8.2));
    float minY=MIN(500, player.position.y-150);
	[_background1 setPosition:ccp(minX,minY)];
    CCTMXLayer *_background2 = [tileMap layerNamed:@"Background2"];
	[_background2 setPosition:ccp(minX,minY)];
    ...
    if(player.position.x>620) {
        [myDefaults setFloat:30.0f forKey:@"startX"];
        [myDefaults setFloat:72.0f forKey:@"startY"];
...
-(void)setViewpointCenter:(CGPoint) position {
    ...
    int x = MAX(position.x, winSize.width / 2+20);
    int y = MAX(position.y+100, winSize.height / 2);
...
-(void)rightButtonDoubleTapped
{
    if(moving==NO){
        ...
        dispatch_after(popTime2, dispatch_get_main_queue(), ^(void){
            [player runAction:chooseAction.jumpStopAction];
            [self createAllPlatformbody];
