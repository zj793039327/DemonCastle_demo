...
//----------------------自定义适应横幕-----------------------------------------------------------------------------------------
    UIWindow *window;
    window = [[UIWindow alloc] initWithFrame:CGRectMake(0,0, 480, 640)];//UIDeviceOrientationLandscapeRight
    //window = [[UIWindow alloc] initWithFrame:CGRectMake(0,0, 480, 320)];//UIDeviceOrientationLandscapeLeft
...
- (void)animateWithType:(URBAlertAnimation)animation show:(BOOL)show completionBlock:(void (^)())completion {
	...
	// tumble animation
		...
		if (show) {
			...
			[UIView animateWithDuration:0.8 delay:0.2 options:UIViewAnimationCurveEaseOut animations:^{
...
- (void)showOverlay:(BOOL)show {
	if (show) {
        ...
        //UIWindow *window = [[UIWindow alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
//----------------------自定义适应横幕-----------------------------------------------------------------------------------------
        UIWindow *window = [[UIWindow alloc] initWithFrame:CGRectMake(0, 0, 480, 480)];
        //[window setTransform:CGAffineTransformMakeRotation(-M_PI/2)];//UIDeviceOrientationLandscapeLeft
        [window setTransform:CGAffineTransformMakeRotation(M_PI/2)];//UIDeviceOrientationLandscapeRight
//---------------------------------------------------------------------------------------------------------------------------
        self.window = window;
		...
		dispatch_async(dispatch_get_main_queue(), ^{
			...
			[UIView animateWithDuration:0.15 delay:0.0 options:UIViewAnimationOptionLayoutSubviews animations:^{
				self.blurredBackgroundView.alpha = 0.6f;
